...
def classificar_edificacao(dados: dict) -> dict:
    ...
    # 5. Determinar sistemas exigidos (precisa ser antes do processo, pois influencia)
    ...
    # 6. Definir tipo de processo (PTS ou Projeto Técnico) — IT-01/IT-42 CBMBA
    populacao_adotada = populacao_informada or populacao_calculada
    sistemas_ids = [s.get("id", "") for s in sistemas]
    exige_hidrante = "HIDRANTES" in sistemas_ids
    processo = definir_tipo_processo(
        area_construida=area,
        altura=altura,
        grupo=grupo,
        divisao=divisao,
        numero_pavimentos=dados.get("numero_pavimentos", 1),
        populacao=populacao_adotada,
        risco=risco["nivel"],
        exige_hidrante=exige_hidrante,
    )
