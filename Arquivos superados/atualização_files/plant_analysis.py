...
def analisar_planta(
    ...
    # Monta os blocos de conteúdo: PLANTA (obrigatória) + MEMORIAL (opcional)
    blocos, erro = _bloco_conteudo(arquivo_bytes, filename, content_type, "planta")
    if erro:
    ...
    if tem_memorial:
        blocos_mem, erro_mem = _bloco_conteudo(
            memorial_bytes, memorial_filename, memorial_content_type, "memorial"
        )
        if erro_mem:
    ...
    instrucao_usuario = (
        "Faça a AUDITORIA de conformidade desta planta de combate a incêndio"
        + (" e do memorial descritivo enviado" if tem_memorial else "")
        + ", conforme as Instruções Técnicas do CBMBA. "
        "Diga se o enquadramento está correto, audite cada sistema exigido (conforme / "
        "não conforme / pendente), aponte divergências e atribua a NOTA (0 a 10) e o STATUS. "
        "Responda APENAS com o JSON conforme o system prompt."
    )
